...
from arm_guard.domain.models import AlertDecision, DetectionFrame, DrowsinessAssessment, SystemHealth
    ...
    def process(self, frame: DetectionFrame) -> ProcessingResult:
        ...
        if not self._consent_manager.can_process(frame.context):
            assessment = DrowsinessAssessment(
                ...
                confidence=1.0,
                reasons=("processing blocked by consent policy",),
                should_alert=False,
            )
        ...
        ear = self._ear_estimator.calculate_average(frame.left_eye, frame.right_eye)
        baseline = self._profiler.update_and_get_baseline(
            driver_id=frame.driver_id,
            ear=ear,
            context=frame.context,
        )
        threshold_multiplier = self._alert_policy.threshold_multiplier(frame.context)
        context_risk_multiplier = self._alert_policy.risk_multiplier(frame.context)
        ...
        microsleep_duration_seconds = blink_duration_seconds if blink_duration_seconds >= 1.5 else 0.0
        ...
        should_alert = self._alert_policy.alert_allowed(frame.context) and drowsiness_score >= 0.55

        reasons: list[str] = []
        if eyes_closed:
            reasons.append("eye aspect ratio below contextual threshold")
        if blink_duration_seconds >= 0.25:
        ...
        health = self._monitor.snapshot(
            acceleration_backend=self._acceleration_profile.summary(),
            camera_online=True,
            fallback_mode=bool(microsleep_duration_seconds),
